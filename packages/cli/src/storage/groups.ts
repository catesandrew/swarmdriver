...
import type { StorageCredentialOptions, SauceStorageItemResponse, SauceStorageListResponse } from './types'

interface ListGroupsActionOptions extends StorageCredentialOptions {
  id: string[]
  kind: string[]
  query?: string
  page?: number
...
interface SauceStorageGroupSettingsResponse {
  identifier?: string
}
...
const list = (program: Command): void => {
  ...
  .action(async () => {
    const opts = cmd.opts() as ListGroupsActionOptions
...
const settings = (program: Command): void => {
  ...
  .action(async (groupId: string) => {
    ...
    .then((json: SauceStorageGroupSettingsResponse) => {
      if (!logVerboseColor(json, {
        log: console.log,
        ...opts,
      })) {
...
export const sauceStorageGroupsApp = (): void => {
...
export function makeStorageGroupsCommand(name = 'groups'): SauceCommand {
