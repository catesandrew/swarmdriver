 ...
 *
 * Derived from the parsers rather than hand-written — see the equivalent
 * `IosAppiumValues` in `machines/ios/appium.ts` for the reasoning. The keys
 * listed explicitly are the W3C/UiAutomator2 capabilities no parser produces;
 * callers supply them through the `...params` spread in
 * `machines/android/native-app.ts` and `machines/android/chrome.ts`.
 */
export interface AndroidAppiumValues extends
  Partial<ReturnType<typeof parseAppiumSettings>>,
  Partial<ReturnType<typeof parseAndroidAppiumSettings>> {
  platformName?: string
