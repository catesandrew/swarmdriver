...
/** Sauce Labs' Safari-specific `sauce:options` keys, as parsed from the env. */
export type SafariSauceCapabilities = ReturnType<typeof parseSafariSauceCapabilities>

interface DesktopSafariOptions extends MachineSetupOptions {
  /** `wdio:safaridriverOptions`, built by `services/safari-driver.ts`. */
...
/**
 * Generate an x86 config used to test web applications in safari
 */
export const desktopSetupLocalSafari = ({
  ...
  safariDriverOptions = {},
  ...opts
}: DesktopSafariOptions = {}): WdioConfig => {
  return {
    capabilities: [
      {
        browserName: 'safari',
...
export const parseSafariSauceCapabilities = (envs: Envs) => {
  return {
...
const buildSauceSafariCapabilities = ({
  ...opts
}: Partial<SafariSauceCapabilities> = {}) => {
  return {
