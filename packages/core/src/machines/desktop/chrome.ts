...
/** Sauce Labs' Chrome-specific `sauce:options` keys, as parsed from the env. */
export type ChromeSauceCapabilities = ReturnType<typeof parseChromeSauceCapabilities>

interface DesktopChromeOptions extends MachineSetupOptions {
  /** `wdio:chromedriverOptions`, built by `services/chrome-driver.ts`. */
...
export const desktopSetupLocalChrome = ({
  ...
  // built by `services/chrome-driver.ts`.
  chromeDriverOptions = {},
  ...params
}: DesktopChromeOptions = {}): WdioConfig => {
  return {
    capabilities: [
      omitBy({
        browserName: 'chrome',
...
const parseChromeSauceCapabilities = (envs: Envs) => {
  return {
...
const buildSauceChromeCapabilities = ({
  ...opts
}: Partial<ChromeSauceCapabilities> = {}) => {
  return {
