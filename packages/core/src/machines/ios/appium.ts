...
/**
 * The value bag every builder in this module reads from.
 *
 * The bulk of it is *derived* rather than hand-written: `parseAppiumSettings()`
 * and `parseIosAppiumSettings()` already encode which environment variable maps
 * to which capability and at what type, so taking their return types keeps this
 * declaration honest by construction — add a variable to the parser and it
 * shows up here with the right type, no second edit required.
 *
 * `Partial<>` because the parsers omit any key whose value matched its default
 * (see `parseBool` in `utils.ts`), so at runtime most of these are absent.
 *
 * The keys listed explicitly are the W3C/XCUITest capabilities that no parser
 * produces — callers hand them in through the `...params` spread in
 * `machines/ios/native-app.ts` and `machines/ios/safari.ts`.
 */
export interface IosAppiumValues extends
  Partial<ReturnType<typeof parseAppiumSettings>>,
  Partial<ReturnType<typeof parseIosAppiumSettings>> {
  platformName?: string
