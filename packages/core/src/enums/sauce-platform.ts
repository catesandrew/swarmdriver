...
const parseSaucePlatformHelper = (val: EnumInput, def: number): EnumEntry | undefined => {
  try {
    if (isString(val)) {
      if (isNum(val)) {
      ...
      return SaucePlatform.props[SaucePlatform[(val as string).toUpperCase()]]
    }
...
export const parseSaucePlatform = (val: EnumInput, def: number = SaucePlatform.UNKNOWN): EnumEntry => {
