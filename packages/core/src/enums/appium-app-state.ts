...
const logHelper = (val: EnumInput, def: number): EnumEntry | undefined => {
  try {
    if (isString(val)) {
      ...
      return AppiumAppState.props[AppiumAppState[(val as string).toUpperCase()]]
    }
...
export const parseAppiumAppState = (val: EnumInput, def: number = AppiumAppState.NOT_INSTALLED): EnumEntry => {
