...
/**
 * Widened to {@link WdioConfig}: each `Device` branch returns a
 * differently-shaped machine config, and an unrecognised device yields
 * `undefined` — behaviour the caller has always relied on.
 */
...
const helperSauceDeviceBrowserTestMode = (device: number): number | undefined => {
  if (device === Device.IOS) {
