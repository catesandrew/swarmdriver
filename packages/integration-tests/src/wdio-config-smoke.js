 ...
 * each package's built ESM entry. (`src/` uses extensionless imports that
 * plain Node ESM cannot resolve — the bundler is what makes them work.)
 */
const CORE_ENTRY = path.join(PACKAGES, 'core/dist/index.js')
const PROVIDERS_ENTRY = path.join(PACKAGES, 'providers/dist/index.js')

const distUrl = (entry) => {
  if (!fs.existsSync(entry)) {
    throw new Error(
      `Missing ${ entry }. Run \`pnpm -r build\` before the config smoke-test.`,
    )
  }
...
/**
 * @returns {string} file URL of `@caps/core`'s built ESM entry.
 */
export function apiModuleUrl() {
  return distUrl(CORE_ENTRY)
}

/**
 * @returns {string} file URL of `@caps/providers`' built ESM entry.
 */
export function providersModuleUrl() {
  return distUrl(PROVIDERS_ENTRY)
}

/**
 * Both dist entries exist, i.e. the smoke matrix is runnable.
 *
 * @returns {boolean} whether `pnpm -r build` has been run.
 */
export function isBuilt() {
  return fs.existsSync(CORE_ENTRY) && fs.existsSync(PROVIDERS_ENTRY)
}
...
export async function runCombination(combo, { tmpDir }) {
  ...
  fs.writeFileSync(confPath, [
    // Side-effect import: registers the saucelabs provider into @caps/core's
    // registry. Without it `remote: 'saucelabs'` resolves to nothing — this
    // line IS the consumer-facing contract the smoke gate is protecting.
    `import ${ JSON.stringify(providersModuleUrl()) }`,
