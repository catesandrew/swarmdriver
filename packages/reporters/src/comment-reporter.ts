 ...
 */
export interface AnyCapabilities {
  testobject_test_report_url?: string
  testobject_device_name?: string
  deviceName?: string
  browserName?: string
  browser?: string
  ...
  alwaysMatch?: AnyCapabilities
  'sauce:options'?: unknown
  tunnelName?: unknown
}

/**
 * `TestStats` as WDIO defines it, plus the ad-hoc fields this reporter
 * stamps onto test instances at runtime (`testName` in `onTestStart`,
 * `ssRoute`/`psRoute` in `onTestFail`/`onTestEnd`) to pass artifact paths
 * along to `getFailureDisplay`.
 */
type TestStatsWithArtifacts = TestStats & {
  testName?: string
  ssRoute?: string
  psRoute?: string
}

/** Sauce Labs-specific fields WDIO's generic `instanceOptions` shape doesn't declare. */
interface SauceInstanceConfig {
  hostname?: string
  user?: string
  key?: string
}

export interface SauceCommentReporterOptions {
  debugMode?: boolean
  logLevel?: string
  mediaRenderTimeout?: number
  outputDir?: string
  // Should all tests be saved, or only from failed tests
  saveAllTests?: boolean
 ...
 */
const sauceAuthenticationToken = (user: string, key: string, sessionId: string): string => {
...
const generateFilename = (envCombo: string, fullName: string, maxTestNameCharacters = 250): string => {
  const date = new Date()
...
const getJobId = (caps: AnyCapabilities = {}): string | undefined => {
 ...
 */
const getSymbol = (state?: string): string => {
...
export default class SauceCommentReporter extends WDIOReporter {
  static reporterName = 'saucecomment'

  private _suiteUids: Set<string>
  private _indents: number
  private _suiteIndents: Record<string, number>
  private _orderedSuites: SuiteStats[]
  private _stateCounts: { passed: number; failed: number; skipped: number }
  private _onlyFailures: boolean
  private _sauceLabsSharableLinks: boolean
  private _screenshotBaseUrl?: string
  isDone: boolean
  private _outputDir: string
  private _saveAllTests: boolean
  private _videoSlowdownMultiplier: number
  private _mediaRenderTimeout: number
  private _excludedActions: string[]
  private _recordAllActions: boolean
  private _maxTestNameCharacters: number
  private _debugMode: boolean
  private _logLevel: string
  private _saveOutputToFile?: boolean
  screenshotPromises: Promise<unknown>[]
  videoPromises: Promise<unknown>[]
  private _testnameStructure: string[]
  private _frameNr: number
  ...
  onAfterCommand(jsonWireMsg: AfterCommandArgs): void {
    ...
    .then((data) => {
      const ssRoute = path.resolve(this._recordingPath as string, `${ filename }.png`)
  ...
  onTestStart(test: TestStatsWithArtifacts): void {
    // Setup filename based on test name and prepare storage directory
  ...
  onTestFail(test: TestStatsWithArtifacts): void {
    this._stateCounts.failed++
    if (!this._outputDir) {
      return
    }
    ...
    .then(([img, xml]) => {
      ...
      if (img) {
        if (Object.prototype.toString.call(img) === '[object String]') {
          promises.push(
            fs.writeFile(ssRoute, (img as unknown as string).replace(/^data:image\/png;base64,/, ''), 'base64')
            .then((result) => {
      ...
      if (xml) {
        if (Object.prototype.toString.call(xml) === '[object String]') {
          promises.push(
            fs.writeFile(psRoute, xml as unknown as string, 'utf8')
            .then((result) => {
  ...
  onTestEnd(test: TestStatsWithArtifacts): void {
    ...
    if (test.state === 'passed' && this._saveAllTests) {
      ...
      .then((img) => {
        ...
        if (img) {
          ...
          } else if (typeof img === 'object' && (img as { status?: string }).status === 'fulfilled') {
            promises.push(
              fs.writeFile(ssRoute, (img as unknown as { value: string }).value.replace(/^data:image\/png;base64,/, ''), 'base64')
              .then((result) => {
                test.ssRoute = ssRoute
                return result
              })
  ...
  onRunnerEnd(runner: RunnerStats): void {
    ...
    .then((results) => {
      ...
      return this._saveOutputToFile ?
        fs.appendFile(
          ...
          'utf8') :
        this.write(`${ (results as string[]).join('\n') }\n`)
    }, (err) => {
      console.log(err)
    })
    ...
    const wrapItUp = (): void => {
      if (!started) {
        clearTimeout(abortTimer as ReturnType<typeof setTimeout>)
        started = true
  ...
  async printReport(runner: RunnerStats): Promise<string[] | undefined> {
    // Don't print non failed tests
    ...
    // kept for parity with the original report preface; currently unused
    const _preface = `${ envCombo } #${ runner.cid }`
  ...
  getTestLink({ jobId, sessionId, capabilities }: { jobId?: string; sessionId?: string; capabilities?: AnyCapabilities } = {}, useSauceLabsSharableLinks = false): string[] {
    const config = this.runnerStat && (this.runnerStat.instanceOptions[sessionId as string] as SauceInstanceConfig | undefined)
    const isSauceJob = (
    ...
    if (isSauceJob && config && config.user && config.key && (jobId || sessionId)) {
      ...
      const sauceLabsSharableLinks = useSauceLabsSharableLinks ?
        sauceAuthenticationToken(config.user, config.key, sessionId as string) :
        ''
   ...
   */
  getHeaderDisplay(envCombo: string, caps: AnyCapabilities = {}): string[] {
  ...
  getEventsToReport(suite: SuiteStats): (HookStats | TestStats)[] {
    ...
    return [
      ...suite.hooksAndTests.filter((item) => {
        return (item as { type?: string }).type === 'test' || Boolean(item.error)
      })
    ]
  }
   ...
   */
  async getFailureDisplay(): Promise<string[]> {
    let failureLength = 0
    const output: string[] = []
    let length = 0
    const suites = this.getOrderedSuites()
    for (const suite of suites) {
      const eventsToReport = this.getEventsToReport(suite) as TestStatsWithArtifacts[]
      for (const test of eventsToReport) {
        if (test.state !== 'failed') {
          continue
        }
   ...
   */
  getEnviromentCombo(capability: AnyCapabilities = {}): string {
...
export const parseSauceCommentSettings = (envs: EnvRecord): SauceCommentReporterOptions => {
  return {
    ...
    ...(existy(parseWhole(envs, 'SAUCECOMMENT_REPORTER_MEDIA_RENDER_TIMEOUT', 5)) && {
      // NB: upstream JS called `parseBool` here (copy/paste from the
      // neighbouring boolean options) even though this key is a numeric
      // timeout and the `existy` guard above already uses `parseWhole` --
      // `parseBool` would silently coerce any real timeout value (e.g. "10")
      // to `undefined` via `yn()`. Fixed to match the guard while converting.
      mediaRenderTimeout: parseWhole(envs, 'SAUCECOMMENT_REPORTER_MEDIA_RENDER_TIMEOUT', 5),
    }),
    ...(existy(parseWhole(envs, 'SAUCECOMMENT_REPORTER_VIDEO_SLOWDOWN_MULTIPLIER', 3)) && {
      videoSlowdownMultiplier: parseWhole(envs, 'SAUCECOMMENT_REPORTER_VIDEO_SLOWDOWN_MULTIPLIER', 3),
    }),
    ...(existy(parseWhole(envs, 'SAUCECOMMENT_REPORTER_MAX_TEST_NAME_CHARACTERS', 250)) && {
      maxTestNameCharacters: parseWhole(envs, 'SAUCECOMMENT_REPORTER_MAX_TEST_NAME_CHARACTERS', 250),
    }),
  }
}

export const buildSauceCommentSettings = (opts: SauceCommentReporterOptions = {}): { reporters: [typeof SauceCommentReporter, Record<string, unknown>][] } => {
  return {
    reporters: [
...
export const setupSauceCommentConfig = ({
  envs,
  ...opts
}: { envs: EnvRecord } & SauceCommentReporterOptions): { reporters: [typeof SauceCommentReporter, Record<string, unknown>][] } => {
