...
export interface ReportPortalSettings {
  token?: string
  endpoint?: string
  launch?: string
  project?: string
  ...
  seleniumCommandsLogLevel?: string
  autoAttachScreenshots?: boolean
  screenshotsLogLevel?: string
...
export interface ReportPortalReporterConfig {
  services: [unknown, Record<string, unknown>][]
  reporters: [unknown, Record<string, unknown>][]
}

export const parseReportPortalSettings = (envs: EnvRecord): ReportPortalSettings => {
  return {
...
export const buildReportPortalSettings = (opts: ReportPortalSettings = {}): ReportPortalReporterConfig => {
  return {
    services: [
      [RpService, {
      }]
...
export const setupReportPortalConfig = ({
  envs,
  ...opts
}: { envs: EnvRecord } & ReportPortalSettings): ReportPortalReporterConfig => {
